...
- A class for creating unit tests in C++. The catch.hpp file allows for TEST_CASE functions to be created, built and ran as desired. This is not owned by me.
...
- Run `sudo make install` and the files will be copied into the includes folder and will create a default directory or you can specify one by doing `make install at=/usr/local/include/your-directory-name`, and the makefile will automatically create the directory if it does not exist.
